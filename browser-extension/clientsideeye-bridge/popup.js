const BRIDGE_URL = "http://127.0.0.1:17373/api/finding";
  ...
  try {
    ...
    for (const f of findings) {
      const body = new URLSearchParams({
        ...
        summary: f.summary || "Control appears client-side disabled/hidden in rendered DOM and may still be triggerable.",
        evidence: f.evidence || "(no evidence)",
        recommendation: "Do not rely on client-side disable/hide state for authorization. Enforce server-side authorization for action endpoints."
      });
    ...
    status.textContent = `Found: ${findings.length}\nSent: ${ok}\nFailed: ${failed}`;
  } catch (e) {
...
function collectFindings() {
  const riskWords = /(save|submit|delete|remove|admin|role|permission|approve|reject|reset|unlock|disable|enable|export|import|grant|revoke|token|key)/i;
  ...
  const actionable = (el) => {
    ...
    if (tag === "a" || tag === "select" || tag === "textarea" || tag === "form") return true;
    if (tag === "input") return ["", "submit", "button", "image", "reset", "password"].includes(type);
    if (el.getAttribute("role") === "button") return true;
    return !!el.getAttribute("onclick") || !!el.getAttribute("formaction");
  };

  const nodes = Array.from(document.querySelectorAll("button,a,input,select,textarea,form,div,span,[role='button']"));
  const out = [];
  ...
  for (const el of nodes) {
    ...
    const attrs = ["id", "name", "type", "data-testid", "class", "aria-disabled", "href", "title", "aria-label"]
      .map((k) => `${k}="${el.getAttribute(k) || ""}"`).join(" ");
